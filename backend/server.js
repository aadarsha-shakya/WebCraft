...
// New API to save or update branding details
app.post('/api/branding', (req, res) => {
    const { userId, primaryColor, brandName, brandLogo, fontFamily, brandFavicon, imageRatio, popupModalMessage } = req.body;
    ...
    db.query(checkSql, [userId], (err, data) => {
        ...
        if (data.length > 0) {
            ...
            const updateSql = `
                UPDATE branding 
                SET primary_color = ?, brand_name = ?, brand_logo = ?, font_family = ?, 
                    brand_favicon = ?, image_ratio = ?, popup_modal_message = ?
                WHERE user_id = ?`;   
            db.query(updateSql, [primaryColor, brandName, brandLogo, fontFamily, brandFavicon, imageRatio, popupModalMessage, userId], (err, result) => {
                if (err) {
            ...
            // Insert new record
            const insertSql = `
                INSERT INTO branding(user_id, primary_color, brand_name, brand_logo, font_family, brand_favicon, image_ratio, popup_modal_message)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
            db.query(insertSql, [userId, primaryColor, brandName, brandLogo, fontFamily, brandFavicon, imageRatio, popupModalMessage], (err, result) => {
                if (err) {
...
// New API to save or update component details
app.post('/api/components', (req, res) => {
    const { userId, topBarText, navigationType, footerContent, facebookUrl, instagramUrl, tiktokUrl } = req.body;
    ...
    db.query(checkSql, [userId], (err, data) => {
        ...
        if (data.length > 0) {
            ...
            const updateSql = `
                UPDATE components 
                SET top_bar_text = ?, navigation_type = ?, footer_content = ?, 
                    facebook_url = ?, instagram_url = ?, tiktok_url = ?
                WHERE user_id = ?`;   
            db.query(updateSql, [topBarText, navigationType, footerContent, facebookUrl, instagramUrl, tiktokUrl, userId], (err, result) => {
                if (err) {
            ...
            // Insert new record
            const insertSql = `
                INSERT INTO components(user_id, top_bar_text, navigation_type, footer_content, facebook_url, instagram_url, tiktok_url)
                VALUES (?, ?, ?, ?, ?, ?, ?)`;
            db.query(insertSql, [userId, topBarText, navigationType, footerContent, facebookUrl, instagramUrl, tiktokUrl], (err, result) => {
                if (err) {
...
// New API to fetch branding details
...
// New API to fetch component details
